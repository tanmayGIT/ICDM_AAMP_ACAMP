...
function [mindist, minind] = ACAMP_1(data, sub_len)
...
% 
% [data_mu(:, 1), data_sig(:, 1), data(:,1)] = mass_pre(data(:, 1), data_len, sub_len);

for k = 1:1:s
    query = data(1:1+sub_len-1);
    target = data(k+1:((k+1+sub_len)-1));
   
    sumQuery = sum(query);
    sumTarget = sum(target);
    
    sumSquareQuery = sum(query.^2);
    sumSquareTarget = sum(target.^2);
    
    product_me = sum(query.*target);
    
    D = 2*sub_len*(1- (  (product_me - ((sumQuery.*sumTarget)/sub_len)) / sqrt((sumSquareQuery - ...
                        ((sumQuery^2)/sub_len)) * (sumSquareTarget - ((sumTarget^2)/sub_len)))  ));
    ...
    for i = 2:1:s-k+1
        ...
        sumQuery = sumQuery - data(i-1) + data(i+sub_len-1);
        sumTarget = sumTarget - data(kplusi-1) + data(kplusi+sub_len-1); 
        
        sumSquareQuery  = sumSquareQuery - data(i-1)^2 + data(i+sub_len-1)^2 ;
        sumSquareTarget = sumSquareTarget - data(kplusi-1)^2  + data(kplusi+sub_len-1)^2; 
...
function [data_mu, data_sig, cleanData] = mass_pre(data, data_len, sub_len)
